...
yearX = yearX(1);                         % assume current year
arg_default('year',yearX);    
...
switch siteID
    ...
    otherwise
        pthSite = sprintf('//vinimet.geog.ubc.ca/projects/Database/yyyy/%s',siteID);     % get the climate data path
end
...
switch siteID
    case {'HOGG','YOUNG'}
        ...
        fig_num = fig_num-1;
    otherwise
        trace_path  = char(fullfile(pthSite,'Flux','flowrate_mean'));
end
trace_legend = [];
trace_units = 'Flow (lpm)'; % flowrate_min needs to be converted from m^3/sec *1000*60 (L/min)
y_axis      = [];
fig_num = fig_num + fig_num_inc;
x = plt_msig( trace_path, ind, trace_name, trace_legend, year, trace_units, y_axis, t, fig_num,1000*60 );
indAxes = indAxes+1; allAxes(indAxes) = gca;
linkaxes(allAxes,'x');
...
switch siteID
    case {'HOGG','YOUNG'}
        trace_path = char(fullfile(pthSite,'Flux','mean_value_LI-7500'),...
                          fullfile(pthSite,'Flux','rssi_77_mean')...
                          );
        trace_legend = char('7500','7700');
    otherwise
        trace_path  = char(fullfile(pthSite,'Flux','avg_signal_strength_7200_mean'),...
                           fullfile(pthSite,'Flux','rssi_77_mean')...
                           );
        trace_legend = char('7200','7700');
end
