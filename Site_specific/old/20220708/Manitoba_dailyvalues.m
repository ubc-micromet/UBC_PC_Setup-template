...
for siteNum = 1:length(siteNames)
    ...
    Fco2 = read_bor(fullfile(pth,'FLUX','co2_flux'),[],[]);
    co2 = read_bor(fullfile(pth,'FLUX','co2_mixing_ratio'),[],[]);
    Fch4 = read_bor(fullfile(pth,'FLUX','ch4_flux'),[],[]);
    Taraw = read_bor(fullfile(pth,'FLUX','TA_1_1_1'),[],[]);
    Hraw = read_bor(fullfile(pth,'FLUX','H'),[],[]);
    LEraw = read_bor(fullfile(pth,'FLUX','LE'),[],[]);
